...
object NoteService {
    ...
    private val comments: MutableList<NoteComment> = mutableListOf()
    private val deletedComments: MutableList<NoteComment> = mutableListOf()
    ...
    fun createComment(noteId: Int, message: String): Int {
        comments += NoteComment(commentId++, noteId, message)
    ...
    fun deleteComment(commentId: Int): Boolean {
        when (val comment = comments.find { it.id == commentId }) {
            null -> throw NoteCommetNotFountException(commentId)
            else -> {
    ...
    fun editComment(commentId: Int, message: String): NoteComment {
    ...
    fun getById(noteId: Int): Note {
        return when (val note = notes.find { it.id == noteId }) {
            null -> throw NoteCommetNotFountException(noteId)
            else -> note
        }
    }

    fun getComments(noteId: Int, count: Int = 20): List<NoteComment> {
